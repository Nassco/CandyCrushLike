export default class Cookie {
  ...
  selectionnee() {
    // Ajout de la classe 'cookie-selected'
    this.htmlImage.classList.add("cookie-selected");

    // Changement de l'image en version highlightée
    const urlHighlight = Cookie.urlsImagesSurlignees[this.type];
    this.htmlImage.src = urlHighlight;
  }

  deselectionnee() {
    // Suppression de la classe 'cookie-selected'
    this.htmlImage.classList.remove("cookie-selected");

    // Retour à l'image normale
    const urlNormale = Cookie.urlsImagesNormales[this.type];
    this.htmlImage.src = urlNormale;
  }
